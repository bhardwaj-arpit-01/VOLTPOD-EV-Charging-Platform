...
const Register = () => {
  ...
  return (
    <div className="min-h-full flex items-center justify-center bg-[#e8ecf2] dark:bg-[#060c1a] relative overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        >
          <defs>
            ...
            <radialGradient id="reg-bg" cx="55%" cy="75%" r="65%">
              <stop offset="0%"   stopColor="#0d1a2e"/>
              <stop offset="100%" stopColor="#04080f"/>
            </radialGradient>
            {/* Cable energy gradient */}
            <linearGradient id="reg-cableGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%"   stopColor="#00ff88"/>
              <stop offset="100%" stopColor="#00d4ff"/>
            </linearGradient>
            {/* Battery fill gradient */}
            <linearGradient id="reg-battGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%"   stopColor="#00ff88"/>
              <stop offset="50%"  stopColor="#00d4ff"/>
              <stop offset="100%" stopColor="#818cf8"/>
            </linearGradient>
            {/* Ground reflection */}
            <linearGradient id="reg-groundGrad" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%"   stopColor="#0d1a2e" stopOpacity="0.6"/>
              <stop offset="100%" stopColor="#04080f" stopOpacity="0"/>
            </linearGradient>
            {/* Car body gradient */}
            <linearGradient id="reg-carGrad" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%"   stopColor="#1e2d45"/>
              <stop offset="100%" stopColor="#0f172a"/>
            </linearGradient>
            {/* Soft glow filter */}
            <filter id="reg-glow" x="-40%" y="-40%" width="180%" height="180%">
              <feGaussianBlur stdDeviation="6" result="blur"/>
              <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
            </filter>
            <filter id="reg-softGlow" x="-30%" y="-30%" width="160%" height="160%">
          ...
          {/* ── Background ── */}
          <rect width="1000" height="580" fill="url(#reg-bg)"/>
          ...
          <g transform="translate(148,270)">
            ...
            {/* Status LED */}
            <circle cx="28" cy="90" r="7" fill="#00ff88" opacity="0.9" filter="url(#reg-softGlow)"/>
            {/* Brand strip */}
          ...
          <g transform="translate(300,290)">
            ...
            <path d="M15,110 L15,170 Q15,182 30,182 L370,182 Q385,182 385,170 L385,110 Z"
                  fill="url(#reg-carGrad)" stroke="#334155" strokeWidth="1.5"/>
            ...
            {/* Front headlight */}
            <path d="M8,120 Q4,132 6,148 L18,145 L18,122 Z" fill="#fef3c7" opacity="0.95" filter="url(#reg-softGlow)"/>

            {/* ── Rear ── */}
            <path d="M378,110 Q388,130 385,165 L370,165 L370,110 Z" fill="#111827" stroke="#1e293b" strokeWidth="1"/>
            {/* Taillight */}
            <rect x="375" y="122" width="8" height="24" rx="3" fill="#ef4444" opacity="0.85" filter="url(#reg-softGlow)"/>

            {/* ── Charging port (left side of car) ── */}
            <rect x="14" y="126" width="18" height="13" rx="3" fill="#00ff88" opacity="0.9" filter="url(#reg-softGlow)"/>
            ...
            <g transform="translate(90, -28)">
              ...
              <motion.rect
                ...
                fill="url(#reg-battGrad)" opacity={0.85}
      ...
      {/* ── SIGNUP CARD ── */}
      <div className="bg-white/85 dark:bg-[#111827]/85 backdrop-blur-2xl p-8 md:p-10 rounded-3xl border border-slate-200 dark:border-gray-800 shadow-xl dark:shadow-[0_0_60px_rgba(0,0,0,0.6)] w-full max-w-lg relative z-10 transition-colors duration-300 my-12">
        <div className="flex justify-center mb-6">
          <div className="w-16 h-16 bg-slate-100 dark:bg-[#0a0f1a] rounded-2xl border border-cyan-300/50 dark:border-[#00d4ff]/30 flex items-center justify-center shadow-[0_0_15px_rgba(0,212,255,0.2)]">
            <Zap className="w-8 h-8 text-cyan-500 dark:text-[#00d4ff]" />
          </div>
        </div>

        <h2 className="text-3xl font-bold font-['Orbitron'] text-center text-slate-800 dark:text-white mb-2">Create Account</h2>
        <p className="text-center text-slate-500 dark:text-gray-400 mb-8">Join the VoltPod network.</p>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-100 dark:bg-[#0a0f1a] border border-slate-200 dark:border-gray-700 rounded-xl py-3 pl-12 pr-4 text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-gray-600 focus:outline-none focus:border-cyan-400 dark:focus:border-[#00d4ff] transition-colors"
              required
            />
          </div>
          <div className="relative">
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-gray-800">
            <h4 className="text-sm font-semibold text-slate-500 dark:text-gray-400 mb-4 uppercase tracking-wider">Vehicle Info (Optional)</h4>
            <div className="grid grid-cols-2 gap-4">
              <div className="relative">
                <Car className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-gray-500" />
                <input
                  type="text"
                  name="vehicleName"
              ...
              <div className="relative">
                <Zap className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-gray-500" />
                <input
                  type="number"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-cyan-400 to-emerald-400 dark:from-[#00d4ff] dark:to-[#00ff88] text-white dark:text-[#0a0f1a] font-bold py-4 rounded-xl text-lg hover:shadow-[0_0_20px_rgba(0,212,255,0.4)] dark:hover:shadow-[0_0_20px_rgba(0,212,255,0.4)] transition-all flex justify-center items-center mt-6"
          >
            {loading
        ...
        <p className="text-center text-slate-500 dark:text-gray-400 mt-8">
          ...
          <Link to="/login" className="text-cyan-600 dark:text-[#00d4ff] font-semibold hover:underline">Login</Link>
        </p>
      </div>
    </div>
  );
